...
<h1 class="md:flex ml-4 mb-8 text-lg font-bold">Add a Product</h1>
...
<form action="{{ route('product.store') }}" method="POST" enctype="multipart/form-data" class="w-full min-w-sm max-w-lg  p-6">
  {{ csrf_field() }}
  <div class="md:flex md:items-center mb-6">
    ...
    <div class="min-w-64">
      <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" 
            name="name" id="name" maxlength="100"  type="text" value="{{ old('name') }}">
    </div>
  </div>
  <div class="md:flex md:items-center mb-6">
    <div class="min-w-56">
    ...
    <div class="min-w-56">
      <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" 
             type="number" name="price" id="price" step=".01" min="0" value="{{ old('price') }}">
    </div>
  </div>
  <div class="md:flex md:items-center mb-6">
    <div class="min-w-56">
    ...
    <div class="min-w-56">
      <input class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" 
             type="number" name="shipping_cost" id="shipping_cost" step=".01" min="0" value="{{ old('shipping_cost') }}">
    </div>
  </div>
    ...
    <div class="md:w-2/3">
        <select name="product_status" id="product_status" class="bg-gray-200 border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500">
            @foreach ($productStatuses as $productStatus)
                <option value="{{ $productStatus->id }}" {{  old('product_status') ==  $productStatus->id ? 'selected' : '' }}> 
                {{ $productStatus->name }} 
                </option>                        
            @endforeach                    
        </select>       
  ...
  <div class="md:flex md:items-center">
    ...
    <div class="md:w-2/3">
      <input type="submit" value="Save Product" class="shadow bg-blue-500 hover:bg-blue-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" type="button">        
    </div>
  </div>
</form>
